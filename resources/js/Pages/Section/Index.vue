    ...
    <div>
        <div class="flex items-center gap-2 mb-8">
            ...
            <Link :href="route('branches.create')"
                  class="flex justify-center w-1/6 bg-white px-2 py-1 border border-gray-300 rounded-lg text-center">+
                Ветка
            </Link>
        </div>
        <div v-if="sections">
            <div v-for="section in sections" class="mb-8">
                <div v-if="section.branches.length">
                    ...
                    <div v-for="branch in section.branches" class="flex items-center">
                        <Link class="flex w-full bg-white p-4 border border-gray-300"
                              :href="route('branches.show', branch.id)">
                            <h3>{{ branch.title }}</h3>
                        </Link>
                        <Link :href="route('branches.edit', branch.id)" class="flex bg-sky-600 p-4">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
